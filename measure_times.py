...
for _ in range(time_measure_num):
    ...
    diff_time = time.time() - temp_time
    print("Finished. {}[s]".format(diff_time))
    time_measure_list.append(diff_time)

import numpy as np
print(f"平均：{np.mean(np.array(time_measure_list))}")
print(f"標準偏差：{np.std(np.array(time_measure_list))}")
