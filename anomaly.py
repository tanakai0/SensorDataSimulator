...
def simulate_MMSE(start, end, step, init_v = 29.5, g = -((29.0-19.5) / 9.0), error_s = None, error_e = 0):
    ...
    for t in new_functions.date_generator(start + step, end, step):
        m, e = np.random.normal(loc = 0, scale = error_s), np.random.normal(loc = 0, scale = error_e)
        mmse += c + m
...
def simulate_values_from_MMSE(MMSE, start, end, step, func):
    ...
    if start < MMSE[0][0] or MMSE[-1][1] < end:
        print("start:{}\nMMSE[0][0]:{}\nMMSE[-1][1]:{}\nend:{}\n".format(start, MMSE[0][0], MMSE[-1][0], end))
        raise ValueError('The input range (from start to end) is out of range of MMSE! Those must be MMSE[0][0] <= start < end <= MMSE[-1][1].')
...
def simulate_state_anomaly_periods(MMSE, mean_freq, mean_d, sd_d):
    ...
    for interval in MMSE:
        ...
        for _ in range(n):
            ...
            ret.append( (timedelta(days = s_day), timedelta(days = s_day) + timedelta(days = d)) )
    return ret
...
def forgetting_labels(AS, WT, forgetting_num):
    ...
    if forgetting_num[-1][1] < AS[0].start or AS[-1].end < forgetting_num[0][0]:
        print('Some periods of forgetting_num are out of range of the activity sequence.')
    forgetting_activities = [act.activity.name for act in AS if act.activity.home_equipment != {}]
    ...
    if len(error_periods) != 0:
        print('Warning: The number of candidates is smaller than the number of forgetting in some period. So, the real number of forgetting is less than the input. You should reduce the number of forgetting of input. Error periods are:')
    ...
    names, costs = [], []
    for i in indexes:
        home_equipment = random.choice(list(AS[i].activity.home_equipment.keys()))  # one of (not all) the home appliances is forgetting to turn off
    ...
    for i in range(len(indexes)):
        ret.append((indexes[i], indexes_2[i], names[i], costs[i], start_times[i], end_times[i]))
    return ret
...
def determine_fall_indexes(AS, fall_parameters, fall_type):
    ...
    if fall_type == 'w':
        if ('num' not in keys) or ('mean_lie_down_seconds' not in keys):
            raise ValueError('parameters[\'num\'] and parameters[\'mean_lie_down_seconds\'] are needed.')
    if fall_type == 's':
        if ('num' not in keys) or ('mean_lie_down_seconds' not in keys) or ('place' not in keys):
            raise ValueError('parameters[\'num\'], parameters[\'mean_lie_down_seconds\'] and parameters[\'place\'] are needed.')
    if fall_parameters['num'][-1][1] < AS[0].start or AS[-1].end < fall_parameters['num'][0][0]:
    ...
    error_periods = []  # the ranges will be added into this if the number of candidates at each period is smaller than the number of falls at the period.
    ...
    for x in fall_parameters['num']:
        if act_start <= x[1] and x[0] <= act_end:
            ...
            for (i, act) in enumerate(AS):
                if fall_type == 'w':
                    if i <= maximum_ind and act.duration > duration_limit and act.place != AS[i+1].place:
                        candidates.append(i)
                elif fall_type == 's':
                    if i <= maximum_ind and act.duration > duration_limit and act.place in fall_parameters['place'] and act.place != AS[i+1].place:
                        candidates.append(i)
            # If the number of candidates at each period is smaller than the number of falls at the period, stop to assign falls during the period and print it's warning.
            if len(candidates) < x[2]:
                error_periods.append((x[0], x[1]))
    ...
    if len(error_periods) != 0:
        print('Warning: The number of candidates is smaller than the number of falls in some period. So, the real number of falls is less than the input. You should reduce the number of falls of input. Error periods are:')
        for x in error_periods:
            print("{} - {}, ".format(x[0], x[1]), end = '')
        print('')
    if len(indexes) == 0:
...
def basic_activity_model_with_anomaly(act_model, day, state_anomaly_labels, anomaly_parameters):
    """
    ...
    if 'wandering_minute' in anomaly_parameters.keys() and 'wandering_num' in anomaly_parameters.keys():
        num = decide_which_parameter(anomaly_parameters['wandering_num'], day)
        num = num / ((anomaly_parameters['wandering_num'][0][1] - anomaly_parameters['wandering_num'][0][0]) / timedelta(days = 1))
        duration = decide_which_parameter(anomaly_parameters['wandering_minute'], day)
        temp_model['necessary_activities'].append(activity_model.wandering.modified_new_activity(num_mean = num, duration_mean = duration, duration_sd = duration / 5))
    ...
    if BEING_HOUSEBOUND in state_anomaly_labels.keys():
        ...
        if is_housebound == True:
            ...
            exist_use_the_phone = activity_model.use_the_phone in temp_model['random_activities']
            temp_model['random_activities'] = [act for act in temp_model['random_activities'] if act.name != activity_model.go_out.name and act.name != activity_model.use_the_phone.name]
            if exist_go_out == True:
                temp_model['necessary_activities'].append(activity_model.go_out_necessary.modified_new_activity(
                    num_mean = anomaly_parameters['housebound_go_out_num'], 
                    duration_mean = anomaly_parameters['housebound_go_out_duration'],
                    duration_sd = anomaly_parameters['housebound_go_out_duration'] / 5))
            if exist_use_the_phone == True:
                temp_model['necessary_activities'].append(activity_model.use_the_phone_necessary.modified_new_activity(
                    num_mean = anomaly_parameters['housebound_use_the_phone_num'], 
                    duration_mean = anomaly_parameters['housebound_use_the_phone_duration'],
                    duration_sd = anomaly_parameters['housebound_use_the_phone_duration'] / 5))
    ...
    if BEING_SEMI_BEDRIDDEN in state_anomaly_labels.keys():
        ...
        if is_semi_bedridden == True:
            # add nap to random activity and make duraion of rest longer
            temp_model['random_activities'].append(activity_model.nap.modified_new_activity(
            ...
            temp_model['random_activities'] = [act for act in temp_model['random_activities'] if act.name != activity_model.rest.name and act.name != activity_model.go_out.name]
            if exist_go_out == True:
                temp_model['necessary_activities'].append(activity_model.go_out_necessary.modified_new_activity(
                    num_mean = anomaly_parameters['semi_bedridden_go_out_num'], 
                    duration_mean = anomaly_parameters['semi_bedridden_go_out_duration'], 
                    duration_sd = anomaly_parameters['semi_bedridden_go_out_duration'] / 5))
            if exist_rest == True:
                temp_model['random_activities'].append(activity_model.rest.modified_new_activity(
                    duration_mean = anomaly_parameters['semi_bedridden_rest_duration'], 
                    duration_sd = anomaly_parameters['semi_bedridden_rest_duration'] / 5))
    return temp_model
...
def decide_which_parameter(time_param_list, day):
    ...
    for x in time_param_list:
        if x[0] <= timedelta(days = day) <= x[1]:
            return x[2]
    raise ValueError("This cannot find appropriate time range on {} day.\nThe range of time_param_list is from {} to {}.\nThe day must be contained in this range.".format(day, time_param_list[0][0], time_param_list[-1][1]))
...
def save_MMSE(path, values, file_name = 'MMSE'):
    ...
    text = ''
    for x in values:
        text += "{:04} [day] {:02} [h] {:02} [m] {:02} [s] {:06} [ms] - {:04} [day] {:02} [h] {:02} [m] {:02} [s] {:06} [ms], value: {}\n".format(*new_functions.get_d_h_m_s_ms(x[0]), *new_functions.get_d_h_m_s_ms(x[1]), str(x[2]))
    text += '\n'
...
def save_housebound_labels(path, housebound_labels, file_name = 'housebound_labels'):
    ...
    for interval in housebound_labels:
        text += "{:04} [day] {:02} [h] {:02} [m] {:02} [s] {:06} [ms] - {:04} [day] {:02} [h] {:02} [m] {:02} [s] {:06} [ms]\n".format(*new_functions.get_d_h_m_s_ms(interval[0]), *new_functions.get_d_h_m_s_ms(interval[1]))
    if not os.path.exists(path):
        os.makedirs(path)
    file_path = path / "{}.txt".format(file_name)
    with open(file_path, "w") as f:
...
def save_anomaly_parameters(path, anomaly_paraters, file_name = 'anomaly_parameters'):
    ...
    for (k, v) in anomaly_paraters.items():
        text += "{}:\n".format(k)
        if (type(v) == type(0)) or (type(v) == type(0.0)):  # int, float
            text += "{}\n".format(str(v))
...
def save_wandering_labels(path, WT, file_name = 'wandering_labels'):
    ...
    for i, wt in enumerate(WT):
        if wt.walking_type == new_functions.WANDERING_WALKING:
            count += 1
            text += "Activity index: {:04}, Start: {:04} [day] {:02} [h] {:02} [m] {:02} [s] {:06} [ms], End: {:04} [day] {:02} [h] {:02} [m] {:02} [s] {:06} [ms], Duration: {:.2f}[m].\n".format(i, *new_functions.get_d_h_m_s_ms(wt.start_time), *new_functions.get_d_h_m_s_ms(wt.end_time), (wt.end_time - wt.start_time)/timedelta(minutes = 1))
...
def save_fall_labels(path, WT, file_name = 'fall_labels'):
    ...
    for i, wt in enumerate(WT):
        if wt.fall_w or wt.fall_s:
            text += "{}-th trajectory ({} - {})\n".format(i, wt.start_place, wt.end_place)
            if wt.fall_w:
                text += "fall while walking:\n"
                text += "{:04} [day] {:02} [h] {:02} [m] {:02} [s] {:06} [ms]\n".format(*new_functions.get_d_h_m_s_ms(wt.timestamp[wt.fall_w_index]))
                text += "lie down: {} [sec.]\n".format(wt.lie_down_seconds_w)
                count_w += 1
            if wt.fall_s:
                text += "fall while standing:\n"
                text += "{:04} [day] {:02} [h] {:02} [m] {:02} [s] {:06} [ms]\n".format(*new_functions.get_d_h_m_s_ms(wt.timestamp[wt.fall_s_index]))
                text += "lie down: {} [sec.]\n".format(wt.lie_down_seconds_s)
                count_s += 1
    text = "Total number of fall while walking: {}, Total number of fall while walking: {}\n".format(count_w, count_s) + text + '\n'
    if not os.path.exists(path):
        os.makedirs(path)
    file_path = path / "{}.txt".format(file_name)
    with open(file_path, "w") as f:
...
def save_forgetting_labels(path, forgetting_labels, file_name = 'forgetting_labels'):
    ...
    for i, x in enumerate(forgetting_labels):
        text += "{}\n".format(i)
        text += "start: {:04} [day] {:02} [h] {:02} [m] {:02} [s] {:06} [ms]\n".format(*new_functions.get_d_h_m_s_ms(x[4]))
        text += "end  : {:04} [day] {:02} [h] {:02} [m] {:02} [s] {:06} [ms]\n".format(*new_functions.get_d_h_m_s_ms(x[5]))
        text += "name: {}, cost: {}\n".format(x[2], x[3])
        count += 1
