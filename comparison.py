...
def load_CASAS_Aruba(path, is_filled = True, go_out_exists = True, delete_short_other = True, rename_eating = True, rename_nap = True, delete_enter_leave = True, delete_toilet = True, delete_respirate = True):
    """
    ...
    def connect_same_activities(act_list):
        ...
        for (s, e) in intervals:
            ret.append((act_list[s][0], new_functions.TimeInterval(act_list[s][1].start, act_list[e][1].end)))
        return ret
    ...
    skip_s = datetime(year = 2011, month = 5, day = 23, hour = 2, minute = 43, second = 44, microsecond = 902903)
    skip_e = datetime(year = 2011, month = 5, day = 23, hour = 21, minute = 44, second = 59, microsecond = 759837)
    ...
    with open(path, mode = 'r') as f:
        ...
        for line in f:  # e.g. line = '2010-11-04 00:03:50.209589 M003 ON Sleeping begin'
            ...
            this_time = None
            if len(seconds) == 1:
                this_time = datetime(year = int(date[0]), month = int(date[1]), day = int(date[2]), 
                                     hour = int(time[0]), minute = int(time[1]), 
                                     second = int(seconds[0]))
            elif len(seconds) == 2:
                while len(seconds[1]) < 6:  # '00:03:50.0209' -> seconds = ['50', '0209000']
                    seconds[1] += '0'
                this_time = datetime(year = int(date[0]), month = int(date[1]), day = int(date[2]), 
                                     hour = int(time[0]), minute = int(time[1]), 
                                     second = int(seconds[0]), microsecond = int(seconds[1]))
                
            if is_contained == True and skip_s <= this_time <= skip_e:
                    continue
            if is_contained == False and this_time == skip_e:
                is_contained = True
            ...
            if is_overlapped == False:  # Activities are not overlapped now.
                if 'begin' in line:
                    if is_filled == True and other_start_time != None:
                        Aruba.append((other, new_functions.TimeInterval(start = other_start_time, end = this_time - start_point)))
                    activity_name = splitted_line[4]
                    start_time = this_time - start_point
                if 'end' in line:
                    end_time = this_time - start_point
                    Aruba.append((activity_name, new_functions.TimeInterval(start = start_time, end = end_time)))
    ...
    Aruba[0] = (Aruba[0][0], new_functions.TimeInterval(start = start_point - start_point, end = Aruba[0][1].end))
    Aruba[-1] = (Aruba[-1][0], new_functions.TimeInterval(start = Aruba[-1][1].start, end = end_point - start_point))
    ...
    threshold_time = timedelta(minutes = 10)  # baseline duration time to delete short activity
            ...
            if Aruba[i][0] == other:
                ...
                if Aruba[i+1][0] == other:
                    if Aruba[i+1][1].duration <= threshold_time:
                        ret.append((Aruba[i][0], new_functions.TimeInterval(start = Aruba[i][1].start, end = Aruba[i+1][1].end)))
                    else:
                        ret.append(Aruba[i])
                else:
                    ret.append(Aruba[i])
    ...
    if go_out_exists:
        ...
        for i in range(1, len(Aruba) - 1):
            if Aruba[i][0] == other and Aruba[i-1][0] == 'Leave_Home' and Aruba[i+1][0] == 'Enter_Home':
                ret.append((go_out, new_functions.TimeInterval(start = Aruba[i][1].start, end = Aruba[i][1].end)))
            else:
                ret.append(Aruba[i])
        ret.append(Aruba[-1])
        Aruba = ret
    ...
    if delete_enter_leave:
        ...
        ret = []
        for (s, e) in intervals:
            ret.append((Aruba[s][0], new_functions.TimeInterval(Aruba[s][1].start, Aruba[e][1].end)))
        Aruba = connect_same_activities(ret)
    ...
    if rename_eating:
        ...
        for a in Aruba:
            n = a[0]
            if n == 'Eating':
                hour = new_functions.get_d_h_m_s_ms(a[1].start)[1]
...
def convert_data_into_days(data):
    ...
    """
    if data[0][1].start != timedelta(days = 0) or data[-1][1].end != timedelta(days = data[-1][1].end.days):
        raise ValueError('data must be just from timedelta(days = 0) to timedelta(days = ?).')
    ...
    for a in data:
        ...
        temp_start = new_functions.extract_time_of_day(a[1].start)
        temp_end = new_functions.extract_time_of_day(a[1].end)
        if a[1].start.days != a[1].end.days:
            ret[temp_day].append((a[0], new_functions.TimeInterval(temp_start, end_of_a_day)))
            if a[1].end.days - a[1].start.days > 1:
                for i in range(a[1].start.days + 1, a[1].end.days):
                    ret[i].append((a[0], new_functions.TimeInterval(start_of_a_day, end_of_a_day)))
            if temp_end != start_of_a_day:
                ret[a[1].end.days].append((a[0], new_functions.TimeInterval(start_of_a_day, temp_end)))
        else:
            ret[temp_day].append((a[0], new_functions.TimeInterval(temp_start, temp_end)))
    return ret
...
def estimate_daily_frequency(data, act_names = None):
    ...
    if data[0][1].start != timedelta(hours = 0):
        raise ValueEroor('Data must starts from timedelta(hours = 0)!')
    daily_freq_mean, daily_freq_sd = {name: None for name in act_names}, {name: None for name in act_names}
...
def make_act2char_dic(act_list):
    ...
    for a in act_list:
        ...
        i += 1
        if i >= len(candidates):
            raise ValueError('Number of activities is greater than the length of candidates of characters!')
    return ret
...
def convert_day_into_str(day, act2char_map, T_d):
    ...
    for t in new_functions.date_generator(timedelta(hours = 0), timedelta(hours = 24), step = T_d):
        interval = new_functions.TimeInterval(t, t + T_d)
...
def different_days_distance(days_X, days_Y, T_d, activity = None, print_progress = True):
    ...
    for i, (Xday_str, Yday_str) in enumerate(itertools.product(Xday_strings, Yday_strings)):
...
def mean_different_days_distance(days_list_1, days_list_2, T_d, print_progress = False):
    ...
    for i, (_days_1, _days_2) in enumerate(zip(days_list_1, days_list_2)):
        temp = different_days_distance(_days_1, _days_2, T_d, print_progress = print_progress)
        mean_list.append(temp[0])
        sd_list.append(temp[1])
...
def estimate_start_time(data, act_names = None):
    ...
    if act_names == None:
        ...
        for a in data:
            if a[0] not in act_names:
                act_names.append(a[0])
    start_hour_mean, start_hour_sd = {x: None for x in act_names}, {x: None for x in act_names}
    for act_name in act_names:
        start_hours = [a[1].start for a in data[1:] if a[0] == act_name]  # exclude first activity because it was cutted to start from 0:00
        polar_x, polar_y = [], []  # polar coordinates
        for start in start_hours:
            temp_hour = new_functions.extract_time_of_day(start) / timedelta(hours = 1)  # [hour from the start of the day]
...
def estimate_duration_time(data, act_names = None):
    ...
    if act_names == None:
        ...
        for a in data:
            if a[0] not in act_names:
                act_names.append(a[0])
    duration_hour_mean, duration_hour_sd = {name: None for name in act_names}, {name: None for name in act_names}
    for act_name in act_names:
        # exclude first activity and end activity because it was cutted to start from 0:00 or finish until 24:00
        duration = [a[1].duration / timedelta(hours = 1) for a in data[1:len(data)-1] if a[0] == act_name]
...
def Aruba_activity_model(n_day = 220, Aruba_path = Path('./aruba/data'), modify_sleep_param = False):
    """
    ...
    for a in Aruba:
        if a[1].end.days < n_day:
            ...
            last_act = (a[0], new_functions.TimeInterval(a[1].start, timedelta(days = n_day)))
    ...
    for a in fundamental_act_names:
        if a not in activity_names:
            raise ValueError("Parameters of fundamental activitiy {} cannot be learned.".format(a))
    start_hour_mean, start_hour_sd = estimate_start_time(Aruba_n_day, fundamental_act_names)
    duration_hour_mean, duration_hour_sd = estimate_duration_time(Aruba_n_day, activity_names)
    ...
    if modify_sleep_param == True:
        sleep_start_sd = timedelta(hours = start_hour_sd['Take_Breakfast'] / 2) / timedelta(minutes = 1)
        sleep_duration_sd = timedelta(hours = duration_hour_sd['Take_Breakfast'] / 2) / timedelta(minutes = 1)
    else:
        sleep_start_sd = timedelta(hours = start_hour_sd['Take_Breakfast']) / timedelta(minutes = 1)
        sleep_duration_sd = timedelta(hours = duration_hour_sd['Take_Breakfast']) / timedelta(minutes = 1)
        
    take_breakfast = activity_model.FundamentalActivity('Take_Breakfast',
                                         ...
                                         ['Dinner_Table_Chair', 'Entrance'], '#ffd700', {})
    take_lunch = activity_model.FundamentalActivity('Take_Lunch',
                                         ...
                                         ['Dinner_Table_Chair', 'Entrance'], '#ffa500', {})
    take_dinner = activity_model.FundamentalActivity('Take_Dinner',
                                         ...
                                         ['Dinner_Table_Chair', 'Entrance'], '#b22222', {})
    sleeping = activity_model.FundamentalActivity('Sleeping',
    ...
    nap = activity_model.NecessaryActivity('Nap', frequency_mean['Nap'],
                            timedelta(hours = duration_hour_mean['Nap']) / timedelta(minutes = 1),
                            timedelta(hours = duration_hour_sd['Nap']) / timedelta(minutes = 1), ['Bed', 'Sofa'], '#7cfc00', {})
    wash_dishes = activity_model.NecessaryActivity('Wash_Dishes', frequency_mean['Wash_Dishes'],
                                    ...
                                    ['Kitchen_Stove'], '#008080', {'faucet in the kitchen':1})
    meal_preparation = activity_model.NecessaryActivity('Meal_Preparation', frequency_mean['Meal_Preparation'],
                                    ...
                                    ['Kitchen_Stove'], '#ff0000', {'stove in the kitchen':10, 'faucet in the kitchen':1})
    housekeeping = activity_model.NecessaryActivity('Housekeeping', frequency_mean['Housekeeping'],
                            timedelta(hours = duration_hour_mean['Housekeeping']) / timedelta(minutes = 1),
                            timedelta(hours = duration_hour_sd['Housekeeping']) / timedelta(minutes = 1), [], '#bc8f8f', {})
    # random activities
    relax = activity_model.RandomActivity('Relax', timedelta(hours = duration_hour_mean['Relax']) / timedelta(minutes = 1),
                                    timedelta(hours = duration_hour_sd['Relax']) / timedelta(minutes = 1), ['Sofa', 'Bed', 'Desk_Chair'], '#d3d3d3', {},
                                    weight = frequency_mean['Relax'])
    go_out = activity_model.RandomActivity('Go_out', timedelta(hours = duration_hour_mean['Go_out']) / timedelta(minutes = 1),
                                    timedelta(hours = duration_hour_sd['Go_out']) / timedelta(minutes = 1), ['Entrance'], '#ffe4c4', {},
                                    weight = frequency_mean['Go_out'])
    work = activity_model.RandomActivity('Work', timedelta(hours = duration_hour_mean['Work']) / timedelta(minutes = 1),
                                    timedelta(hours = duration_hour_sd['Work']) / timedelta(minutes = 1), ['Desk_Chair', 'Desk'], '#808000', {},
                                    weight = frequency_mean['Work'])
    other = activity_model.RandomActivity('Other', timedelta(hours = duration_hour_mean['Other']) / timedelta(minutes = 1),
                                    timedelta(hours = duration_hour_sd['Other']) / timedelta(minutes = 1), [], '#800000', {},
    ...
    Aruba_act_model = {
        ...
        'fundamental_activity_order': [take_breakfast, take_lunch, take_dinner, sleeping],
...
def generate_random_activity_sequence(start_day, end_day, name_list, mean_list, sd_list):
    """
    ...
    while t < end_time:
        ...
        while is_valid == False:
            ...
            if try_count >= max_try:
                raise ValueError("This failed to sample duration time of {} {} times.".format(self.__name, try_count))
        duration_time = timedelta(minutes = duration)
        ret.append((name_list[index], new_functions.TimeInterval(t, t + duration_time)))
        t += duration_time
    ret[-1] = (ret[-1][0], new_functions.TimeInterval(ret[-1][1].start, timedelta(end_day + 1)))
    return ret
...
def Aruba_activity_model_sleep_only(n_day = 220, Aruba_path = Path('./aruba/data')):
    ...
    sleeping = activity_model.FundamentalActivity('Sleeping',
                                         ...
                                         ['Bed'], '#696969', {})
    # random activities
    take_breakfast = activity_model.RandomActivity('Take_Breakfast',
                                                   timedelta(hours = duration_hour_mean['Take_Breakfast']) / timedelta(minutes = 1),
                                                   timedelta(hours = duration_hour_sd['Take_Breakfast']) / timedelta(minutes = 1),
                                                   ['Dinner_Table_Chair', 'Entrance'], '#ffd700', {})
    take_lunch = activity_model.RandomActivity('Take_Lunch',
                                               timedelta(hours = duration_hour_mean['Take_Lunch']) / timedelta(minutes = 1),
                                               timedelta(hours = duration_hour_sd['Take_Lunch']) / timedelta(minutes = 1),
                                               ['Dinner_Table_Chair', 'Entrance'], '#ffa500', {})
    take_dinner = activity_model.RandomActivity('Take_Dinner',
                                                timedelta(hours = duration_hour_mean['Take_Dinner']) / timedelta(minutes = 1),
                                                timedelta(hours = duration_hour_sd['Take_Dinner']) / timedelta(minutes = 1),
                                                ['Dinner_Table_Chair', 'Entrance'], '#b22222', {})
    nap = activity_model.RandomActivity('Nap',
                                        timedelta(hours = duration_hour_mean['Nap']) / timedelta(minutes = 1),
                                        timedelta(hours = duration_hour_sd['Nap']) / timedelta(minutes = 1),
                                        ['Bed', 'Sofa'], '#7cfc00', {})
    wash_dishes = activity_model.RandomActivity('Wash_Dishes',
                                                timedelta(hours = duration_hour_mean['Wash_Dishes']) / timedelta(minutes = 1),
                                                timedelta(hours = duration_hour_sd['Wash_Dishes']) / timedelta(minutes = 1),
                                                ['Kitchen_Stove'], '#008080', {'faucet in the kitchen':1})
    meal_preparation = activity_model.RandomActivity('Meal_Preparation',
                                                     timedelta(hours = duration_hour_mean['Meal_Preparation']) / timedelta(minutes = 1),
                                                     timedelta(hours = duration_hour_sd['Meal_Preparation']) / timedelta(minutes = 1),
                                                     ['Kitchen_Stove'], '#ff0000', {'stove in the kitchen':10, 'faucet in the kitchen':1})
    housekeeping = activity_model.RandomActivity('Housekeeping',
                                                 timedelta(hours = duration_hour_mean['Housekeeping']) / timedelta(minutes = 1),
                                                 timedelta(hours = duration_hour_sd['Housekeeping']) / timedelta(minutes = 1),
                                                 [], '#bc8f8f', {})
    relax = activity_model.RandomActivity('Relax',
                                          timedelta(hours = duration_hour_mean['Relax']) / timedelta(minutes = 1),
                                          timedelta(hours = duration_hour_sd['Relax']) / timedelta(minutes = 1),
                                          ['Sofa', 'Bed', 'Desk_Chair'], '#d3d3d3', {})
    go_out = activity_model.RandomActivity('Go_out', 
                                           timedelta(hours = duration_hour_mean['Go_out']) / timedelta(minutes = 1),
                                           timedelta(hours = duration_hour_sd['Go_out']) / timedelta(minutes = 1), 
                                           ['Entrance'], '#ffe4c4', {})
    work = activity_model.RandomActivity('Work',
                                         timedelta(hours = duration_hour_mean['Work']) / timedelta(minutes = 1),
                                         timedelta(hours = duration_hour_sd['Work']) / timedelta(minutes = 1), 
                                         ['Desk_Chair', 'Desk'], '#808000', {})
    other = activity_model.RandomActivity('Other',
    ...
    Aruba_act_model = {
        ...
        'random_activities':[take_breakfast, take_lunch, take_dinner, nap, wash_dishes, meal_preparation, housekeeping, relax, go_out, work, other],
...
def load_kasterenActData(path, first_date = datetime(year = 2008, month = 2, day = 26), last_date = datetime(year = 2008, month = 3, day = 20),
    ...
    with open(path, mode = 'r') as f:
        for (i, line) in enumerate(f):
            if 5 <= i <= 11:  # id and activity
                act_map[line[:line.find(':')]] = line[line.find("'")+1: line.rfind("'")]
            if 15 <= i <= 259:  # data
                splitted_line = line.split()
                start_time = datetime(year = int(splitted_line[0].split('-')[2]),
                ...
                end_time = datetime(year = int(splitted_line[2].split('-')[2]),
                ...
                raw_data.append((act_name, new_functions.TimeInterval(start = start_time, end = end_time)))         
    ...
    for (a, interval) in raw_data:
        if first_date < interval.end and is_in == False:
            is_in = True
            data_24.append((a, new_functions.TimeInterval(first_date - first_date, interval.end - first_date)))
        elif end_date < interval.end and is_in == True:
            data_24.append((a, new_functions.TimeInterval(interval.start - first_date, end_date - first_date)))
            break
        elif is_in == True and first_date < interval.start and interval.end < end_date:
            data_24.append((a, new_functions.TimeInterval(interval.start - first_date, interval.end - first_date)))
    ret = data_24
    ...
    if add_other == True:
        ...
        if ret[0][1].start != first_date - first_date:
            other_data.append((other, new_functions.TimeInterval(first_date - first_date, ret[0][1].start - first_date)))
        end_index = len(ret) - 1
        _temp_end = ret[0][1].end  # end time of past one activity or past multiple activities that overlap
        _other_start, _other_end = ret[0][1].end, None  # start time and end time of other activity
        i = 1
        while i <= end_index:
            now_start, now_end = ret[i][1].start, ret[i][1].end  # start time and end time of present acitivity
            if now_start < _temp_end:
                ...
                other_data.append((other, new_functions.TimeInterval(_other_start, _other_end)))
                _temp_end = now_end
            i += 1
        if data_24[-1][1].end != end_date - first_date:
            other_data.append((other, new_functions.TimeInterval(ret[-1][1].end - first_date, end_date - first_date)))
    ...
    if remove_exception == True:
        for (i, a) in enumerate(ret):
            if (a[0] == 'leave house' and a[1].duration / timedelta(hours = 1) > 23) or \
               (a[0] == 'get drink' and a[1].start < timedelta(days = a[1].start.days, hours = 12) ) or \
               (a[0] == 'go to bed' and a[1].duration / timedelta(hours = 1) < 6):
                ret[i] = (exception, new_functions.TimeInterval(start = a[1].start, end = a[1].end))
...
def kasteren_activity_model(kasteren, n_day = None, modify_sleep_param = True):
    ...
    for a in fundamental_act_names:
        if a not in activity_names:
            raise ValueError("Parameters of fundamental activitiy {} cannot be learned.".format(a))
    start_hour_mean, start_hour_sd = estimate_start_time(kasteren_n_day, fundamental_act_names)
    duration_hour_mean, duration_hour_sd = estimate_duration_time(kasteren_n_day, activity_names)
    frequency_mean, _ = estimate_daily_frequency(kasteren_n_day, activity_names)

    take_breakfast = activity_model.FundamentalActivity('prepare Breakfast',
                                         ...
                                         ['Dinner_Table_Chair', 'Entrance'], '#ffd700', {})
    take_shower = activity_model.FundamentalActivity('take shower',
                                         ...
                                         ['Bathroom_Door'], '#00ffff', {'faucet in the bathroom':1})
    take_dinner = activity_model.FundamentalActivity('prepare Dinner',
    ...
    sleep_duration_sd = timedelta(hours = duration_hour_sd['go to bed']) / timedelta(minutes = 1)
    if modify_sleep_param == True:
        sleep_start_sd /= 3
        sleep_duration_sd /= 3
    sleeping = activity_model.FundamentalActivity('go to bed',
                                         ...
                                         sleep_duration_sd,
                                         ['Bed'], '#696969', {})
    use_toilet = activity_model.NecessaryActivity('use toilet', frequency_mean['use toilet'],
                            timedelta(hours = duration_hour_mean['use toilet']) / timedelta(minutes = 1),
                            timedelta(hours = duration_hour_sd['use toilet']) / timedelta(minutes = 1), ['Toilet_Door'], '#4b0082', {})
    ...
    go_out = activity_model.RandomActivity('leave house',
                                           timedelta(hours = duration_hour_mean['leave house']) / timedelta(minutes = 1),
                                           timedelta(hours = duration_hour_sd['leave house']) / timedelta(minutes = 1),
                                           ['Entrance'], '#ffe4c4', {},
                                           weight = frequency_mean['leave house'])
    get_drink = activity_model.RandomActivity('get drink',
    ...
    kasteren_act_model = {
        ...
        'fundamental_activity_order': [take_breakfast, take_shower, take_dinner, sleeping],
        # The order of the fundamental activities: the smaller activity's index is, the earlier the activity is done. 
        'sleep':[sleeping]
    }
...
def kasteren_activity_model_sleep_only(kasteren, n_day = None, modify_sleep_param = True):
    ...
    sleeping = activity_model.FundamentalActivity('go to bed',
                                         ...
                                         sleep_duration_sd,
                                         ['Bed'], '#696969', {})
    take_breakfast = activity_model.RandomActivity('prepare Breakfast',
                                                   timedelta(hours = duration_hour_mean['prepare Breakfast']) / timedelta(minutes = 1),
                                                   timedelta(hours = duration_hour_sd['prepare Breakfast']) / timedelta(minutes = 1),
                                                   ['Dinner_Table_Chair', 'Entrance'], '#ffd700', {},
                                                   weight = frequency_mean['prepare Breakfast'])
    take_shower = activity_model.RandomActivity('take shower',
                                                timedelta(hours = duration_hour_mean['take shower']) / timedelta(minutes = 1),
                                                timedelta(hours = duration_hour_sd['take shower']) / timedelta(minutes = 1),
                                                ['Bathroom_Door'], '#00ffff', {'faucet in the bathroom':1}, 
                                                weight = frequency_mean['take shower'])
    take_dinner = activity_model.RandomActivity('prepare Dinner',
                                                timedelta(hours = duration_hour_mean['prepare Dinner']) / timedelta(minutes = 1),
                                                timedelta(hours = duration_hour_sd['prepare Dinner']) / timedelta(minutes = 1),
                                                ['Dinner_Table_Chair', 'Entrance'], '#ffd700', {},
                                                weight = frequency_mean['prepare Dinner'])
    use_toilet = activity_model.RandomActivity('use toilet',
                                               timedelta(hours = duration_hour_mean['use toilet']) / timedelta(minutes = 1),
                                               timedelta(hours = duration_hour_sd['use toilet']) / timedelta(minutes = 1),
                                               ['Toilet_Door'], '#4b0082', {}, weight = frequency_mean['use toilet'])
    ...
    kasteren_act_model = {
        'fundamental_activities':[sleeping],
        'necessary_activities':[],
        'random_activities':[take_breakfast, take_shower, take_dinner, use_toilet, go_out, get_drink, other],
        'fundamental_activity_order': [sleeping],
        # The order of the fundamental activities: the smaller activity's index is, the earlier the activity is done. 
        'sleep':[sleeping]
    }
