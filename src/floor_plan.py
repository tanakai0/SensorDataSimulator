...
class FloorPlan:
    ...
    def __init__(self):
        ...
        self.Poss_Hs = {
            self.wardrobe.name: [140, 155, 170],
            self.nightstand.name: [45, 50, 55],
            self.desk.name: [60, 65, 70],
            self.chair.name: [40, 45],
            self.dinner_table_chair.name: [40, 45],
            self.desk_chair.name: [40, 45],
            self.sofa.name: [40, 45],
            self.TV.name: [40, 45, 50, 55, 60, 65, 70],
            self.dinner_table.name: [60, 65, 70],
            self.kitchen_stove.name: [75, 80, 85],
            self.cupboard.name: [100, 105, 110, 115, 120],
            self.refrigerator.name: [125, 145, 165],
            self.wash_machine.name: [85],
            self.trash_bin.name: [30, 45, 60],
        }
        ...
        # from FSave
        self.Foldername = {
            self.wardrobe.name: [0, "WAR"],
            self.desk.name: [1, "De"],
            self.sofa.name: [2, "So"],
            self.dinner_table.name: [3, "DTA"],
            self.kitchen_stove.name: [4, "KS"],
            self.cupboard.name: [5, "CB"],
            self.refrigerator.name: [6, "RFA"],
            self.wash_machine.name: [7, "WM"],
            self.trash_bin.name: [8, "TB"],
        }
        self.layout_save_folder = None

        # from Plot
        self.Code = {
            self.toilet.name: self.toilet.plot_name,
            self.bathroom.name: self.bathroom.plot_name,
            self.bed.name: self.bed.plot_name,
            self.nightstand.name: self.nightstand.plot_name,
            self.wardrobe.name: self.wardrobe.plot_name,
            self.desk.name: self.desk.plot_name,
            self.chair.name: self.chair.plot_name,
            self.dinner_table_chair.name: self.dinner_table_chair.plot_name,
            self.desk_chair.name: self.desk_chair.plot_name,
            self.sofa.name: self.sofa.plot_name,
            self.TV.name: self.TV.plot_name,
            self.dinner_table.name: self.dinner_table.plot_name,
            self.kitchen_stove.name: self.kitchen_stove.plot_name,
            self.cupboard.name: self.cupboard.plot_name,
            self.refrigerator.name: self.refrigerator.plot_name,
            self.wash_machine.name: self.wash_machine.plot_name,
    ...
    def convert_furniture_into_furs(self, furs):
        ...
        for i in range(3):
            if furs[i][-1]:
                n_fur = [furs[i][j] + 1 for j in range(len(furs[i]) - 1)]
            else:
                n_fur = [2 * furs[i][j] for j in range(len(furs[i]) - 1)]
            n_furs.append(n_fur)
        return n_furs
    ...
    def generate_layout_with_necessary_furniture(
        self,
        layout_database_folder,
        folder_name=None,
        topology="RCLR",
        ...
        for _ in range(max_iter):
            ret = self.generate_layout(
                self.layout_database_folder,
                folder_name,
                topology,
                furniture,
                save=False,
            )
            if ret and set(necessary_furniture).issubset(set(self.existing_places())):
                return self.save_layout_data(
                    self.layout_database_folder / self.folder_name
                )
        return None
    ...
    def generate_layout(
        ...
        if topology not in ["RC", "RL", "RCL", "RLC", "RCLR", "random"]:
            raise ValueError(
                "topology must be in ['RC', 'RL', 'RCL', 'RLC', 'RCLR', 'random']."
            )
    ...
    def generate_floor_plan(self):
        ...
        def sizes_sample(
            ...
            if not se_is_K:
                L_L = B_L
                L_W, L_add = si_side(
                    L_A_min, L_A_max, len_step, L_L, kitchen=False, To_W=To_W
                )
                t = np.random.rand()
                ...
                if t < inds[0] / ind2k:
                    ...
                    if L_L > B_L_max or (L_L + To_W) >= 2 * L_W:
                        L_add = False
                    if (L_L + To_W) * L_W > L_A_max:
                        L_add = False
        ...
        def TB_LD(_type, B_L, B_W, K_W, L_W, L_L, Size, K_cut):
            ...
            if _type == 1:
                l_1 = [
                    [B_ri, B_up - Ba_L],
                    [B_ri, B_up],
                    [B_ri, B_up + L_W - Ba_L - To_L],
                ]
                l_2 = [[B_ri, B_up], [B_ri, B_up + L_W - Ba_L - To_L]]
                if L_W < (To_L + Ba_L):
                    l_1, l_2 = [[B_ri, B_up - Ba_L]], []
                ...
                if L_W < (To_L + Ba_L):
                    l_1, l_2 = [[B_ri, B_up - Ba_L]], [[B_ri, B_up + K_W - To_L]]
                ...
                l_2 = [[B_ri, B_up], [B_ri, B_up + L_W - Ba_L - To_L]]
                if L_W < (To_L + Ba_L):
                    l_1, l_2 = [[B_ri, B_up - Ba_L], [B_ri, B_up + L_W - Ba_L]], []
                l_3 = []
                l_4 = []
            elif _type == 4:
                l_1 = [
                    [B_le - Ba_W, B_up - Ba_L],
                    [B_ri + L_L, B_up + K_W - L_W],
                    [B_ri + L_L, B_up + K_W - Ba_L - To_L],
                ]
                # Attention on l_1[0]
                l_2 = [
                    [B_ri + L_L, B_up + K_W - L_W],
                ...
                l_3 = [
                    [B_ri, B_up + K_W],
                    [B_ri + L_L - Ba_L - To_L, B_up + K_W],
                    [B_ri, B_up + K_W - L_W - To_W],
                ...
                if L_L < (To_L + Ba_L):
                    l_3, l_4 = [], [[[B_ri - To_L, B_up + K_W]]]
            elif _type == 5:
                l_1 = [
                    [B_le - Ba_W, B_up - Ba_L],
                    [B_ri + L_L, B_do],
                    [B_ri + L_L, B_do + L_W - Ba_L - To_L],
                ]
                l_2 = [[B_ri + L_L, B_do], [B_ri + L_L, B_do + L_W - Ba_L - To_L]]
                l_3 = [
                    [B_ri - Ba_L, B_do - Ba_W],
                    [B_ri, B_do - Ba_W],
                    [B_ri + L_L - Ba_L - To_L, B_do - Ba_W],
                    [B_ri, B_do + L_W],
        ...
        def rect_com(_type, B_L, B_W, K_W, L_L, L_W):
            ...
            Kitchen, Livingroom = [0, 0, 0, 0, self.kitchen.name], [
                0,
                0,
                0,
                0,
                self.livingroom.name,
            ]
            if _type == 0:
        ...
        def cut_add(
            ...
            if L_add:
                if _type < 4:
                    L_L += To_W
                    if (
                        T_con == self.livingroom.name or B_con == self.livingroom.name
                    ) and (T_B_in or (not is_y)):
                        L_x -= To_W
                    else:
                        if t_l[1]:
                            L_x -= To_W
        ...
        def Pla_Door(_type, Bedr, Kitc, Livi, Toil, Bath, DW=60):
            ...
            def wall_and_Obs(x_y_lims, bounds, _type):
                def Cut(strings0, strings1, yD):
                    ...
                    if strings1 != []:
                        ...
                        if p01[yD] <= p10[yD] or p00[yD] >= p11[yD]:
                            ...
                            [p00, p01], [p10, p11] = strings0.pop(0), strings1.pop(0)
                        if (
                            p00[yD] < p10[yD] < p01[yD]
                            or p00[yD] < p11[yD] < p01[yD]
                            or p10[yD] < p00[yD] < p11[yD]
                            or p10[yD] < p01[yD] < p11[yD]
                        ):
                            if p10[yD] > p00[yD]:
                ...
                def Cut_TB_R(x_y_lims, bounds, is_T):
                    ...
                    x1, y1 = (
                        bounds[4 - is_T][-1][-1][-1][0],
                        bounds[4 - is_T][-1][-1][-1][1],
                    )
                    for i in range(3):
                        for j in range(2):
                            for k in range(2):
                ...
                def add_obs_area(X_con, interL, W=self.Pass_W // 2):
                    if interL != [[0, 0], [0, 0]]:
                        ...
                        if p00 == p10:
                            X_con.append([[p00 - W, p01], [p10 + W, p11]])
                        elif p01 == p11:
                            X_con.append([[p00, p01 - W], [p10, p11 + W]])
            ...
            c = sample(Choices)
            J = sample(Weights[c])
            [x, y], i_ = Po_D_cen[c][J][0], Po_D_cen[c][J][1]
            if i_ == 1:
        ...
        def add_TB_obs(Be_con, Ki_con, Li_con, Toil, Bath, T_con, B_con, W):
            ...
            To_temp = [
                [Toil[0] - W, Toil[1] - W],
                [Toil[0] + Toil[2] + W, Toil[1] + Toil[3] + W],
            ]
            Ba_temp = [
                [Bath[0] - W, Bath[1] - W],
                [Bath[0] + Bath[2] + W, Bath[1] + Bath[3] + W],
            ]
        ...
        def TB_Door(Bedr, Kitc, Livi, Toil, Bath, T_con, B_con, TB_W=120, DW=60):
            ...
            if Toil[2] == TB_W:
                T_Door = (
                    [
                        Toil[0],
                        Toil[1] + Toil[3] // 2 - DW // 2,
                        0,
                        DW,
                        self.toilet_door.name,
                    ]
                    if T_R_C[0] < Toil[0]
                    else [
                        Toil[0] + Toil[2],
                ...
                B_Door = (
                    [
                        Bath[0],
                        Bath[1] + Bath[3] // 2 - DW // 2,
                        0,
                        DW,
                        self.bathroom_door.name,
                    ]
                    if B_R_C[0] < Bath[0]
                    else [
                        Bath[0] + Bath[2],
        ...
        if _type > 3 and L_add:
            L_W = L_W - self.To_W
        TB_x, TB_y, is_y, To_on, T_B_in = TB_LD(
            _type, B_L, B_W, K_W, L_W, L_L, self.Size, K_cut
        )
        To_L, To_W, Ba_L, Ba_W, To_x, To_y, Ba_x, Ba_y = loc_TB(
            self.Size, TB_x, TB_y, is_y, To_on
        )
        Bedroom, Kitchen, Livingroom = rect_com(_type, B_L, B_W, K_W, L_L, L_W)
        T_con = who_con(
            _type, To_x, To_y, Bedroom[0], Bedroom[1], Kitchen[1], Livingroom[1]
        )
        B_con = who_con(
            _type, Ba_x, Ba_y, Bedroom[0], Bedroom[1], Kitchen[1], Livingroom[1]
        )
        Kitchen, Livingroom = cut_add(
            _type,
            is_y,
            T_B_in,
            T_con,
        ...
        Toil_Bath = [Toilet, Bathroom]
        Doors, Walls, Be_con, Ki_con, Li_con = Pla_Door(
            _type, Bedroom, Kitchen, Livingroom, Toilet, Bathroom
        )
        ...
        Be_con, Ki_con, Li_con = add_TB_obs(
            Be_con, Ki_con, Li_con, Toilet, Bathroom, T_con, B_con, self.Pass_W
        )
        T_Door, B_Door = TB_Door(
            Bedroom, Kitchen, Livingroom, Toilet, Bathroom, T_con, B_con
        )
        Doors.append(T_Door)
        Doors.append(B_Door)
        ...
        furs = self.furs
        B_F = self.PLF_B(
            furs[0], Bedroom, Bathroom, Be_con, Doors, self.B_A0, self.B_A1
        )
    ...
    def PLF_B(self, fur_B, Bedr, Bath, B_con, Doors, B_A0, B_A1):
        ...
        def Cal_Ai(Room, TB, R_A0, R_A1, TB1=None):
            ...
            if Room[0] < TB_cen[0] < (Room[0] + Room[2]) and Room[1] < TB_cen[1] < (
                Room[1] + Room[3]
            ):
                T_or_B_A = TB[2] * TB[3]
                R_A -= T_or_B_A
            if TB1:
                ...
                if Room[0] < TB1_cen[0] < (Room[0] + Room[2]) and Room[1] < TB1_cen[
                    1
                ] < (Room[1] + Room[3]):
        ...
        def pla_f(exist, room, F_size, F_name, room_con, far_X=0, far_Y=0):
            def poss_strings(room, F_size, far_X, far_Y):
                ...
                XYS = [
                    [R_x + X + far_X, R_x + R_L - X - far_X],
                    [R_y + Y + far_Y, R_y + R_W - Y - far_Y],
                ]
                    ...
                    for i in range(len(strings)):
                        ...
                        if string[0][0] == string[1][0]:
                            if area[0][0] < string[0][0] < area[1][0]:
                                if (
                                    area[0][1] >= string[1][1]
                                    or area[1][1] <= string[0][1]
                                ):
                                    strings.append(string)
                                else:
                                    if area[1][1] < string[1][1]:
                                        strings.append(
                                            [[string[0][0], area[1][1]], string[1]]
                                        )
                                    if string[0][1] < area[0][1]:
                                        strings.append(
                                            [string[0], [string[1][0], area[0][1]]]
                                        )
                            else:
                                strings.append(string)
                        else:
                            if area[0][1] < string[0][1] < area[1][1]:
                                if (
                                    area[0][0] >= string[1][0]
                                    or area[1][0] <= string[0][0]
                                ):
                                    strings.append(string)
                                else:
                                    if area[1][0] < string[1][0]:
                                        strings.append(
                                            [[area[1][0], string[0][1]], string[1]]
                                        )
                                    if string[0][0] < area[0][0]:
                                        strings.append(
                                            [string[0], [area[0][0], string[1][1]]]
                                        )
                            else:
                                strings.append(string)
            ...
            def select_f_c(strings0, strings1):
                ...
                def place_f_c(strings, strings_len, T_len):
                    ...
                    if t < 0.4:
                        ...
                        return [
                            (string[0][0] + string[1][0]) // 2,
                            (string[0][1] + string[1][1]) // 2,
                        ]
            ...
            for rect in room_con:
                ...
                temp_con0.append(
                    [
                        [x0 - L0 - far_X, y0 - W0 - far_Y],
                        [x1 + L0 + far_X, y1 + W0 + far_Y],
                    ]
                )
                temp_con1.append(
                    [
                        [x0 - L1 - far_X, y0 - W1 - far_Y],
                        [x1 + L1 + far_X, y1 + W1 + far_Y],
                    ]
                )
        ...
        def Pla_Nstands(NS_e, Bedr, Bed, Doors, N=self.Nstand_S):
            ...
            [NL, NW] = N
            N_S_centers = [
                [[Bx - NL // 2, By + NW // 2], [Bx - NL // 2, By + BW - NW // 2]],
                [[Bx + NL // 2, By - NW // 2], [Bx + NL - NL // 2, By - NW // 2]],
                [
                    [Bx + BL + NL // 2, By + NW // 2],
                    [Bx + BL + NL // 2, By + BW - NW // 2],
                ],
                [
                    [Bx + NL // 2, By + BW + NW // 2],
                    [Bx + NL - NL // 2, By + BW + NW // 2],
                ],
            ]
            ...
            for i in range(4):
                ...
                if Dist[i] >= 40:
                    ...
                    N_S_centers.append([[], []])
            if Bed[2] == 210:
                N_S_cen = [N_S_centers[1], N_S_centers[3]]
            else:
                N_S_cen = [N_S_centers[0], N_S_centers[2]]
            if t == 1:
                N_S_Cp = []
                for i in range(2):
                    for P in N_S_cen[i]:
        ...
        def Pla_Wri_T_C(T_C, DW=50, CL=40, CW=40):
            ...
            Desks = [
                [x, y, L, DW, self.desk.name],
                [x, y + W - DW, L, DW, self.desk.name],
                [x, y, DW, W, self.desk.name],
                [x + L - DW, y, DW, W, self.desk.name],
            ]
            Chairs = [
                [x + L // 2 - CL // 2, y + W - CW, CL, CW, self.desk_chair.name],
                [x + L // 2 - CL // 2, y, CL, CW, self.desk_chair.name],
                [x + L - CW, y + W // 2 - CL // 2, CW, CL, self.desk_chair.name],
                [x, y + W // 2 - CL // 2, CW, CL, self.desk_chair.name],
            ]
            if T_C[2] > T_C[3]:
                if i:
                    Desk, Chair = Desks[1], Chairs[1]
                else:
                    Desk, Chair = Desks[0], Chairs[0]
            elif T_C[3] > T_C[2]:
                if i:
                    Desk, Chair = Desks[3], Chairs[3]
                else:
                    Desk, Chair = Desks[2], Chairs[2]
            else:
        ...
        if De_e > 0:
            ...
            Wri_T_C = pla_f(
                De_e, Bedr, Wri_T_C_Size, "Wri_T_C", B_con, far_X=far_X, far_Y=far_Y
            )
            if Wri_T_C:
        ...
        def Pla_Din_Cs(Dinner_T, Livi, Doors, far=10, CL=40, CW=40):
            def sam_cha_num(L):
                ...
                if L == self.Dinner_T_S[0][0]:
                    ...
                    if t < 0.15:
                        ...
                        return 3 if t < 0.85 else 4
                elif L == self.Dinner_T_S[3][0]:
                    if t < 0.25:
                        return 2
                    else:
                        return 3 if t < 0.75 else 4
                else:
            ...
            for i in range(4):
                ...
                if Dist[i] >= 50:
                    ...
                    if Dist_D >= 90:
                        Chairs_cen_P.append(center)
            if n > len(Chairs_cen_P):
                n = len(Chairs_cen_P)
        ...
        def Pla_Sofa_TV(L_center, S_TV, S_W=80):
            ...
            if L > W:
                if abs(x - xl) > abs(x + L - xl):
                    Sofa, TV = [x + L - S_W, y, S_W, W, self.sofa.name], [
                        x,
                        y + W // 2 - TV_L // 2,
                        TV_W,
                        TV_L,
                    ...
                    Sofa, TV = [x, y, S_W, W, self.sofa.name], [
                        x + L - TV_W,
                        y + W // 2 - TV_L // 2,
                        TV_W,
                        TV_L,
                ...
                if abs(y - yl) > abs(y + W - yl):
                    Sofa, TV = [x, y + W - S_W, L, S_W, self.sofa.name], [
                        x + L // 2 - TV_L // 2,
                        y,
                        TV_L,
                        TV_W,
                    ...
                    Sofa, TV = [x, y, L, S_W, self.sofa.name], [
                        x + L // 2 - TV_L // 2,
                        y + W - TV_W,
                        TV_L,
                        TV_W,
        ...
        for i in range(len(House)):
            ...
            for furniture in Furnitures[i]:
                ...
                if N1 not in Json_File[N]["Furnitures"]:
                    ...
                    Json_File[N]["Furnitures"][N1]["Size"].append([x, y, L, W])
        Json_File["Doors"] = {}
        for door in Doors:
    ...
    def sample_height_data(self, abs_path):
        ...
        def furnitureH(f_H, House, T_B, Furnitures):
            ...
            def fur_height(na, loca, arrs, has_nist):
                ...
                if na == self.bed.name:
                    if has_nist:
                        ...
                        rj = random.random()
                        if rj < 0.35:
                            f_height = self.Bed_H[0]
                        else:
                            f_height = random.choice(self.Bed_H)
                else:
            ...
            def Sofa_TV_height(S_T, arrs):
                ...
                def Sample_TV_Size(L, W):
                    LL = max(L, W)
                    if LL == 60:
                        return random.choice(self.TV_Sizes[:2])
                    if LL == 80:
                        return random.choice(self.TV_Sizes[:3])
                    if LL == 100:
                        return random.choice(self.TV_Sizes[1:4])
                    if LL == 120:
                        return random.choice(self.TV_Sizes[2:])
            ...
            if len(Furnitures[0]) > 1:
                if Furnitures[0][1][4] == self.nightstand.name:
                    Has_NS = 1
            height_1 = []
            for room in House:
        ...
        # sample_height_data--------------------------------------------------------------------------------
        _type, House, Toil_Bath, Furnitures = (
            self.type,
            self.House,
            self.Toil_Bath,
            self.Furnitures,
    ...
    def save_layout_figure(
        ...
        if save:
            plt.savefig(
                folder_path / (file_name + ".png"), bbox_inches="tight", dpi=500
            )
        if show:
            plt.show()
        if close:
    ...
    def save_height_figure(self, folder_path, file_name="Height_Function", show=False):
        ...
        if masked_v is None:
            ...
            Z = np.ma.masked_greater(data.T, masked_v)
    ...
    def save_distance(self, folder_path):
        ...
        def T_B2obj_W(T_B, rooms):
            ...
            def com_T_B(T_B):
                ...
                # along with y axis
                if x0 == x1:
                    return [x0, min(y0, y1), L0, W0 + W1, "T_B_con"]
                # along with x axis
                else:
                    return [min(x0, x1), y0, L0 + L1, W0, "T_B_con"]
            ...
            for n in range(2):
                ...
                for lim in room_lims:
                    if lim[0][0] < cx < lim[0][1] and lim[1][0] < cy < lim[1][1]:
                        flag[n] = 1
                if flag[n] == 0:
                    for i in range(4):
                        for lim in room_lims:
                            if (
                                lim[0][0] <= wall_cens[i][0] <= lim[0][1]
                                and lim[1][0] <= wall_cens[i][1] <= lim[1][1]
                            ):
                                T_B_walls.append(walls[i])
                                break
            if flag == [0, 0]:
                return None, T_B_walls
            if flag == [1, 0]:
                return T_B[0], T_B_walls
            if flag == [0, 1]:
                return T_B[1], T_B_walls
            return com_T_B(T_B), None

        def mod_walls(walls, T_B_Walls):
            # T_B_Walls are added to walls.
            if T_B_Walls is not None:
        ...
        def cal_dis_val(rooms, furnitures, walls, lims, t_b_in=None):
            ...
            def cal_p_xy(room, i, j):
                ...
                [x, y] = room[:2]
                return [
                    x + self.g_L / 2 + self.g_L * i,
                    y + self.g_W / 2 + self.g_W * j,
                ]
            ...
            def sub_dis_val_c(d, _type="obj"):
                ...
                """
                if _type == "wall":
                    return max(math.sqrt(250 / d) - 1, 0)
                ...
                for i in range(L // self.g_L):
                    for j in range(W // self.g_W):
                        ...
                        if disw < 25:
                            ...
                            sub_dis_val[i][j] += sub_dis_val_c(disw, _type="wall")
            ...
            for n in range(len(rooms)):
                ...
                for i in range(N_L):
                    for j in range(N_W):
                        data[i + N_i][j + N_j] = dis_val[n][i][
                            j
                        ]  # indexes of this matrix do not correspond with (x,y) coordinate
            return data
        ...
        def cal_destinations(rooms, furnitures, doors, dis_val, X0, Y0):
            ...
            Bed_c, Kit_c, Des_c, Tab_c, TV_c = calc_key_centers(
                rooms, furnitures
            )  # center points
            Destinations = {}
            for i in range(len(rooms)):
                ...
                for furniture in furnitures[i]:
                    ...
                    for [k, t] in nodes:
                        if dis_val[k][t] < 99:
                            Destinations[key].append(
                                [k, t]
                            )  # destination points around the furniture
            for door in doors:
                x, y, L, W, d_n = door
                key = d_n
                Destinations[key] = []
        ...
        def weighted_dijkstra(s_nodes, topo_graph, discomfort):
            ...
            def real_distance(P0, P1):
                ...
                [I0, J0], [I1, J1] = P0, P1
                return math.sqrt(
                    self.g_L * self.g_L * (I1 - I0) * (I1 - I0)
                    + self.g_W * self.g_W * (J1 - J0) * (J1 - J0)
                )
        ...
        max_diss_d = (
            {}
        )  # max distance, thus, it corresponds to max LTJ1(x, the destinations) mentioned in [1]
        connectflag = 0
        ...
        for key in destinations.keys():
            ...
            if connectflag == 0:
                ...
                if che_con:
                    ...
                    raise ValueError(
                        "{} CAN NOT be arrived in {}!".format(e_f, folder_path)
                    )
    ...
    def canvas(self, canvas, point2canvas):
        ...
        for room in self.Toil_Bath:
            ...
            canvas.create_rectangle(
                top_left[0], top_left[1], bottom_right[0], bottom_right[1], width=3
            )
        ...
        for room in self.Furnitures:
            for furniture in room:
                ...
                text_pos = point2canvas((x + L / 2, y + W / 2))
                canvas.create_rectangle(
                    top_left[0], top_left[1], bottom_right[0], bottom_right[1], width=1
                )
                canvas.create_text(text_pos[0], text_pos[1], text=self.Code[name])
