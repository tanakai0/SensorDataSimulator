...
class ActivityDataPoint:
    ...
    def __str__(self):
        return "<ActivityDataPoint> {} from {} to {} at {}".format(
            self.activity, self.start, self.end, self.place
        )
...
def generate_activity_sequence(
    ...
    is_changed = (
        False  # the flag of difference between act_model and original_act_model
    )
    ...
    if is_changed:
        print(
            "The original_act_model was modified to activity_model.determine_activity_set(path, original_act_model), so that it can be applied in the layout."
        )
    ...
    for s, e in intervals:
        connected_sequence.append(
            (temp_sequence[s][0], temp_sequence[s][1], temp_sequence[e][2])
        )
...
def activity_generator(
    ...
    first_date.fill_up_schedule_with_basic_activity_model(
        act_model, act_model["sleep"][0], timedelta(hours=6, minutes=30)
    )
    ...
    for day in range(start_day, end_day + 1):
        ...
        while not is_valid:
            ...
            if anomaly_parameters == dict():
                ...
                temp_act_model = anomaly_model.basic_activity_model_with_anomaly(
                    act_model, day, state_anomaly_labels, anomaly_parameters
                )
            try:
                next_date.fill_up_schedule_with_basic_activity_model(
                    temp_act_model,
                    last_activity,
                    extra_time_of_last_activity,
                    sleep_after_24=sleep_after_24,
                )
            except:
                try_count += 1
                if try_count >= max_try:
                    raise ValueError(
                        "It fails to sample {} day {} times. Please try again. One of the reason cause from over sleeping so that taking breakfast at next date can't do. One solution of this is reduce sleep time duration or move sleep time forward.".format(
                            day, try_count
                        )
                    )
            else:
...
class day_schedule:
    ...
    def __init__(self, num_days):
        ...
        self.__remainder = [
            (
                timedelta(days=self.__num_days, hours=0),
                timedelta(days=self.__num_days, hours=24),
            )
        ]
    ...
    def update_schedule_with_multi(self, act_list, is_test=False, is_fundamental=False):
        ...
        if not is_test:
            ...
            if not is_fundamental:
                ...
                for act in act_list:
                    if not self.update_schedule(*act, is_test=True):
                        return False
    ...
    def fill_up_schedule_with_basic_activity_model(
        self,
        ...
        self.update_schedule(
            first_activity, start_of_today, start_of_today + extra_time
        )

        # update if the start time of last sleep is over 0:00
        if sleep_after_24 is not None:
            self.update_schedule(
                sleep_after_24[0],
                start_of_today + sleep_after_24[1],
                start_of_today + sleep_after_24[2],
            )
        ...
        while not is_valid:
            ...
            for act in basic_activity_model["fundamental_activities"]:
                ...
                if isinstance(act, activity_model.Activity):
                    ...
                    update_activities.append(
                        (act, start_time, start_time + duration_time)
                    )
                ...
                fundamental_act_info.append(
                    (act, start_time, start_time + duration_time)
                )

            # check if the activities are not overlap with the last sleep
            is_valid = self.update_schedule_with_multi(
                fundamental_act_info, is_test=True, is_fundamental=True
            )
            # check the order of the fundamental activities
            is_valid *= activity_model.FundamentalActivity.check_order(
                fundamental_act_info, basic_activity_model["fundamental_activity_order"]
            )
            ...
            if sleep_after_24 is not None:
                if (
                    start_of_today
                    + sleep_after_24[2]
                    + timedelta(minutes=threshold_min)
                    > fundamental_act_info[0][1]
                ):
                    is_valid = False
            else:
                if (
                    start_of_today + extra_time + timedelta(minutes=threshold_min)
                    > fundamental_act_info[0][1]
                ):
                    is_valid = False
            for i in range(len(fundamental_act_info) - 1):
                if (
                    fundamental_act_info[i][2] + timedelta(minutes=threshold_min)
                    > fundamental_act_info[i + 1][1]
                ):
                    is_valid = False
        ...
        for x in update_activities:
            ...
            if (
                x[0].name == basic_activity_model["sleep"][0].name
                and end_of_today <= x[1]
            ):
        ...
        for act in random.sample(temp_list, len(temp_list)):  # shuffled order
            ...
            if isinstance(act, activity_model.Activity):
                for _ in range(act_num):
                    ...
                    start_time = None
                    if (
                        not act.exist_repulsive_constraints
                        and not act.exist_existence_constraints
                    ):
                        start_time = act.sampling_start_time(
                            duration_time, self.__remainder
                        )
                    else:
                        start_time = act.sampling_start_time_with_constraints(
                            duration_time, self.__remainder, self.__schedule
                        )
                ...
                for _ in range(act_num):
                    ...
                    sub_activities = [
                        (x[0], x[1] + start_time, x[2] + start_time)
        ...
        while len_remainder != 0:
            ...
            act = activity_model.RandomActivity.sampling_random_activity(
                basic_activity_model["random_activities"]
            )
            duration_time = None
            sub_activities = []
            if isinstance(act, activity_model.Activity):
            ...
            # if the start time of sleep is over 24:00
            if (
                self.__sleep_after_24 is not None
                and end_of_today <= sub_range[0] + duration_time
            ):
                if (
                    sub_range[0] + duration_time + rejection_range
                    > end_of_today + self.__sleep_after_24[1]
                ):
                    if isinstance(act, activity_model.Activity):
                        self.update_schedule(
                            act, sub_range[0], end_of_today + self.__sleep_after_24[1]
                        )
                    elif isinstance(act, activity_model.MetaActivity):
                        continue
                    else:
                    ...
                    if isinstance(act, activity_model.Activity):
                        self.update_schedule(
                            act, sub_range[0], sub_range[0] + duration_time
                        )
                    elif isinstance(act, activity_model.MetaActivity):
                        self.update_schedule_with_multi(sub_activities)
                    else:
...
def save_activity_sequence(path, AS, file_name="activity_sequence"):
    ...
    for act in AS:
        text += "{:04} {:02} {:02} {:02} {:06} {}\n".format(
            *get_d_h_m_s_ms(act.start), act.activity.name
        )
...
def save_activity_sequence_as_image(
    path,
    AS,
    file_name="activity_sequence",
    mask_activities=None,
    aspect_ratio=1,
    ncol=1,
    show=False,
    swap_axies=False,
    ...
    labels = dict()
    for a in AS:
        if (a.activity.name not in labels.keys()) and (
            a.activity.name not in mask_activities
        ):
    ...
    for i, acts in enumerate(AS_days):
        for a in acts:
            if a[0].name in mask_activities:
                continue
            start = y_len * (
                extract_time_of_day(a[1]).total_seconds() / one_day_seconds
            )
            if a[2] == timedelta(days=i + 1):
                end = y_len
            else:
                end = y_len * (
                    extract_time_of_day(a[2]).total_seconds() / one_day_seconds
                )
...
def save_activity_pie_chart(path, AS, dpi=400):
    ...
    for day in range(AS[0].start.days, AS[-1].start.days + 1):
        ...
        days_PM = [
            x for x in days if 12 <= get_d_h_m_s_ms(x[2])[1] or x[2].days == day + 1
        ]
        ...
        for j, focused in enumerate([days_AM, days_PM]):
            ...
            pie_label = [
                x[0].name
                + " ({:.1f} [min])".format((x[2] - x[1]) / timedelta(minutes=1))
                for x in focused
            ]
            ...
            if j == 0:
                plt.savefig(
                    str(path) + "/day{}_AM.png".format(day),
                    bbox_inches="tight",
                    dpi=dpi,
                )
            else:
                plt.savefig(
                    str(path) + "/day{}_PM.png".format(day),
                    bbox_inches="tight",
                    dpi=dpi,
                )
            plt.close()
...
class WalkingTrajectory:
    ...
    def __init__(
        ...
        fall_w_index=None,
        fall_s_index=None,
        lie_down_seconds_w=0,
        lie_down_seconds_s=0,
        fall_w_body_range=40,
    ...
    def __str__(self):
        if self.intermediate_places == []:
            return "<WalkingTrajectory> type: {} from {} ({}) to {} ({})".format(
                self.walking_type,
                self.start_place,
                self.start_time,
                self.end_place,
            ...
            return (
                "<WalkingTrajectory> type: {} from {} ({}) (, {}, ) to {} ({})".format(
                    self.walking_type,
                    self.start_place,
                    self.start_time,
                    ", ".join(self.intermediate_places),
                    self.end_place,
                    self.end_time,
                )
            )
    ...
    def save_text(self):
        ...
        text += "start: {:04} [day] {:02} [h] {:02} [m] {:02} [s] {:06} [ms]\n".format(
            *get_d_h_m_s_ms(self.start_time)
        )
        text += "end: {:04} [day] {:02} [h] {:02} [m] {:02} [s] {:06} [ms]\n".format(
            *get_d_h_m_s_ms(self.end_time)
        )
        text += "place: {} - {} ({}), ".format(
            self.start_place, self.end_place, self.walking_type
        )
...
def generate_walking_trajectories(
    layout_path,
    AS,
    stride,
    step_speed,
    prefer_foot,
    fall_w_parameters=dict(),
    fall_s_parameters=dict(),
):
    """
    ...
    discom = fp.load_file(layout_path, constants.DISCOMFORTABLE_VALUE, "csv")
    destinations = fp.load_file(layout_path, constants.DESTINATIONS, "json")
    max_diss = fp.load_file(layout_path, constants.MAX_DISTANCE, "json")

    distances = {}
    for place in existing_places:
        distances[place] = fp.load_file(layout_path, place + "_distance", "csv")
    edge, g_L, g_W = fp.edge, fp.g_L, fp.g_W
    WT = sample_walking_trajectories(
        AS,
        distances,
        discom,
        destinations,
...
def sample_walking_trajectories(
    AS,
    distances,
    discom,
    destinations,
    ...
    fall_w_indexes, fall_s_indexes = (
        [],
        [],
    )  # candidates of falls, some indexes may be excluded when the erronerous walking trajectoires occur
    if fall_w_parameters != dict():
        fall_w_indexes = anomaly_model.determine_fall_indexes(
            AS, fall_w_parameters, "w"
        )
    if fall_s_parameters != dict():
        fall_s_indexes = anomaly_model.determine_fall_indexes(
            AS, fall_s_parameters, "s"
        )
    ...
    while i < limit:
        ...
        if last_position is None:
            ...
            start_position_indexes = [
                coordinate2index(*last_position, edge, g_L, g_W, lims)
            ]
        ...
        if (AS[i].activity.name != constants.WANDERING_NAME) and (
            start_place == end_place
        ):  # not walking
            if (
                i in fall_w_indexes
                or i in fall_s_indexes
                or AS[i].activity.name == constants.WANDERING_NAME
            ):
                raise ValueError(
                    "The resident is not walking now. Falls or a wandering cannot be happend in 0 steps."
                )
            walking_type = constants.NOT_WALKING
            start_time = end_time
            timestamp = []
            WT.append(
                WalkingTrajectory(
        ...
        if AS[i].activity.name != constants.WANDERING_NAME:  # walking
            walking_type = constants.DIRECT_WALKING
            is_valid = False
            count = 0
            while not is_valid:
                try:
                    (temp_centers, left_steps, right_steps) = sample_path(
                        start_position_indexes,
                        end_place,
                        distances,
                        discom,
                    ...
                    start_time = end_time - timedelta(
                        seconds=(len(centers) - 1) * step_speed
                    )
                    if start_time <= AS[i].start:
                        raise ValueError(
                            "The duration time of the {}-th activity is less than the duration time of {}-th walking to move between places. Try to increase duration time of short activities.".format(
                                i, i
                            )
                        )

        if AS[i].activity.name == constants.WANDERING_NAME:  # wandering
            ...
            walking_type = constants.WANDERING_WALKING
            is_valid = False
            count = 0
            candidates = [
                c for c in place_list if (c != start_place) and (c != end_place)
            ]
            # ``candidates`` are the candidates of places where the resident can reach in a wandering.
            max_seconds = AS[i].duration / timedelta(
                seconds=1
            )  # maximum seconds of a wandering
                ...
                try:  # generate a wandering pattern during the duration time of the wandering
                    while True:
                        if temp_centers_list != []:
                            ...
                            start_position_indexes = [
                                coordinate2index(
                                    temp_lp[0], temp_lp[1], edge, g_L, g_W, lims
                                )
                            ]
                        temp_end_place = random.choice(
                            [c for c in candidates if c != locations[-1]]
                        )
                        (temp_centers, temp_left_steps, temp_right_steps) = sample_path(
                            start_position_indexes,
                            temp_end_place,
                            distances,
                            discom,
                    ...
                    while True:
                        ...
                        if walking_seconds + temp_seconds > max_seconds:
                            if len(locations) == 1:
                                raise ValueError(
                                    "The duration time of the wandering is less than the duration time of walking to move between places."
                                )
                            locations.pop()
                    ...
                    is_valid = True
                    temp_centers = list(
                        itertools.chain.from_iterable(temp_centers_list)
                    )
                    left_steps = list(
                        itertools.chain.from_iterable(temp_left_steps_list)
                    )
                    right_steps = list(
                        itertools.chain.from_iterable(temp_right_steps_list)
                    )
        ...
        timestamp = [
            end_time - timedelta(seconds=i * step_speed) for i in range(len(centers))
        ]  # normal walk by a constant speed without falls
        timestamp.reverse()
        ...
        if (i in fall_w_indexes) and (len(centers) != 0):  # when falls occur
            fall_w = True
            fall_w_sec = np.random.normal(
                loc=fall_w_parameters["mean_lie_down_seconds"],
                scale=fall_w_parameters["mean_lie_down_seconds"] / 5,
            )
            ...
            #     raise ValueError("{}- walking trajectory is too short to fall down.".format(i))
                ...
                timestamp = [
                    x if fall_point_w <= i else x - lie_down_duration_w
                    for i, x in enumerate(timestamp)
                ]
                timestamp.insert(
                    fall_point_w, timestamp[fall_point_w] - lie_down_duration_w
                )

        if (i in fall_s_indexes) and (
            len(centers) != 0
        ):  # sometimes, a fall while walking and a fall while standing are both happended in one walking trajectory.
            fall_s = True
            fall_s_sec = np.random.normal(
                loc=fall_s_parameters["mean_lie_down_seconds"],
                scale=fall_s_parameters["mean_lie_down_seconds"] / 5,
            )
            ...
            timestamp = [
                x if fall_point_s <= i else x - lie_down_duration_s
                for i, x in enumerate(timestamp)
            ]
            timestamp.insert(
                fall_point_s, timestamp[fall_point_s] - lie_down_duration_s
            )
        ...
        WT.append(
            WalkingTrajectory(
                ...
                fall_w=fall_w,
                fall_w_index=fall_point_w,
                lie_down_seconds_w=fall_w_sec,
                fall_s=fall_s,
                fall_s_index=fall_point_s,
                lie_down_seconds_s=fall_s_sec,
            )
        )
        if len(centers) != 0:
...
def sample_path(
    ...
    if (len(HPath) == 1) and (len(Angles) == 0):
        ...
        centers, left_steps, right_steps = normal_bcfp(
            HPath, Angles, stride, lims, prefer_foot, edge, g_L, g_W
        )
...
def direct_path(
    ...
    def next_step(distance, discomfort, location, stride, angle):
        ...
        mean_discomfort = (
            discomfort[I][J]
            + discomfort[row_index, column_index]
            + discomfort_on_half_position
        ) / 3
...
def normal_bcfp(Hpath, Angles, stride, lims, Preferfoot, edge, g_L, g_W):
    ...
    def footprint(Hpath, Angles, stride, PreferFoot):
        ...
        def calinstep(Body_c, angle_, flags):
            ...
            if flags[0]:
                footprints[0].append([I0_l, J0_l])
            if flags[1]:
                footprints[1].append([I0_r, J0_r])
        ...
        for i in range(N - 1):
            if An_diff[i] < math.pi / 4:
                ...
                ang = (
                    Angles[i] + An_diff[i] / 2
                )  # for the case when real delta angle is ang_dif
                if (
                    abs(
                        min(
                            abs(ang - Angles[i + 1]),
                            2 * math.pi - abs(ang - Angles[i + 1]),
                        )
                        - An_diff[i] / 2
                    )
                    > 0.001
...
def save_walking_trajectoires(path, WT, file_name=constants.WALKING_TRAJECTORY):
    """
...
def update_states_of_motion_sensors_in_walks(
    ...
    wt : list of WalkingTrajectory or None
        Walking trajectories.
    sampling_seconds : float
        Sampling duration [s] of sensors.
    sync_reference_point : datetime.timedelta, default timedelta(days = 0)
    ...
    # Under step is disregarded if the sensors are updated while activities. 
    # See the function update_states_of_motion_sensors_in_actvities.
...
def define_synchronous_sampling_point(reference_point, start_time, sampling_seconds):
    ...
    """
    ret = timedelta(
        seconds=((start_time - reference_point).total_seconds() // sampling_seconds)
        * sampling_seconds
        + reference_point.total_seconds()
    )
...
def generate_cost_sensor_data(
    ...
    sampling_start = define_synchronous_sampling_point(
        sync_reference_point, AS[0].start, sampling_seconds
    )
...
def save_layout(
    ...
    if filename == "":
        ...
        if sensors != []:
            sensor_types = [s.type_name for s in sensors]
            sensor_types = list(
                dict.fromkeys(sensor_types)
            )  # delete duplicated type_name
...
def generate_six_anomalies(path, save_path, days, show=True):
    ...
    forgetting_num = [
        (x[0], x[1], stats.poisson.rvs(x[2])) for x in forgetting_mean_num
    ]
...
def pickle_load(path, name):
    ...
    Attempts to load from '.pickle' file first; if not found, tries '.pickle.gz'.
    ...
    pickle_path = path / f"{name}.pickle"
    gzipped_pickle_path = path / f"{name}.pickle.gz"
    loaded_object = None

    if pickle_path.exists():
        with open(pickle_path, "rb") as f:
            loaded_object = pickle.load(f)
    elif gzipped_pickle_path.exists():
        with gzip.open(gzipped_pickle_path, "rb") as gz:
            loaded_object = pickle.load(gz)
    else:
        raise FileNotFoundError(f"No file found for {name} in {path} as .pickle or .pickle.gz")
...
class TimeInterval:
    ...
    def overlap(self, target_interval, exclude_only_1_point=False):
        ...
        if not isinstance(target_interval, TimeInterval):
            raise ValueError("target_interval must be a instance of TimeInterval.")
        if exclude_only_1_point:
            return target_interval.end > self.start and self.end > target_interval.start
        else:
            return (
                target_interval.end >= self.start and self.end >= target_interval.start
            )
    ...
    def overlap_length_with(self, target_interval):
        ...
        if not isinstance(target_interval, TimeInterval):
            raise ValueError("target_interval must be a instance of TimeInterval.")
        if not self.overlap(target_interval):
            return timedelta(hours=0)
        elif self.include_TimdeInterval(target_interval):
            return target_interval.duration
        elif self.is_included_in(target_interval):
            return self.duration
        elif target_interval.end < self.end:
            return target_interval.end - self.start
        elif self.start < target_interval.start:
            return self.end - target_interval.start
        else:
            print(
                "self = ({}, {})\ntarget_interval = ({}, {})".format(
                    self.start, self.end, target_interval.start, target_interval.end
                )
            )
            raise ValueError(
                "There are some situations that TimeInterval.overlap_length_with cannot consider."
            )
