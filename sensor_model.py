...
FLOW_KITCHEN = 'water flow sensor at kitchen'
FLOW_BATHROOM = 'water flow sensor in bathroom'
POWER_TV = 'electric energy meter of TV'
POWER_KITCHEN = 'electric energy meter at kitchen'

def generate_test_sensor_arrange(path, th_discomfort = 90.0, num_PIR = 20, num_pressure = 3, min_d = 50.0, max_iter = 1000):
    """
    ...
    while ((num < num_PIR + num_pressure) and (i < max_iter)):
        ...
        p = (x, y)
        I = int((x - lims[0][0] + fp.edge) // fp.g_L)
        J = int((y - lims[1][0] + fp.edge) // fp.g_W)
        
        if discomfort[I][J] >= th_discomfort:
            is_valid = False
        for _p in positions:
    ...
    if (i >= max_iter) and (num < num_PIR + num_pressure):
        print("The number of sensors is {} and it is less than the input num_PIR + num_pressure.".format(num))
        
    sensors = []
    for i, p in enumerate(positions):
        if i < num_PIR:
            sensors.append(CircularPIRSensor('normal PIR', i, p[0], p[1], '#ff0000', 50))
        else:
            sensors.append(SquarePressureSensor('vertical oblong pressure', i, p[0] - 50, p[1] - 50, '#696969', 100, 100, 0))
    return sensors
...
class Sensor:
    ...
    def __str__(self):
        return "<Sensor>{}(#{})({:.2f}, {:.2f})".format(self.name, self.index, self.x, self.y)
...
class CircularPIRSensor(Sensor):
    ...
    def __init__(self, name, index, x, y, color, radius):
        ...
        if radius < 0:
            raise ValueError('The radius of a CircluarPIRSensor must be positive value.')
    ...
    def draw(self, ax):
        ...
        ax : matplotlib.pyplot.gca
            Current axes on this figure.
        """
        circle = patches.Circle(xy = (self.x, self.y), radius = self.radius, fc = self.color, ec = self.color, alpha = 0.2)
        ax.add_patch(circle)
    ...
    def save_text(self):
        ...
        ret : str
            Text of this sensor.
        """
        return "name = {}, index = {}, center coordinates = ({}, {}), radius = {}[cm]".format(self.name, self.index, self.x, self.y, self.radius)
...
class SquarePressureSensor(Sensor):
    ...
    def __init__(self, name, index, x, y, color, vertical, horizontal, angle):
        ...
        if (vertical < 0) or (horizontal < 0):
            raise ValueError('The vertical length and horizontal length of CircularSensor must be positive value.')
    ...
    def cover(self, xy):
        ...
        (x, y) = temp_xy
        return self.x <= x <= self.x + self.horizontal and self.y <= y <= self.y + self.vertical
    ...
    def collide(self, xy, body_radius):
        ...
        if self.angle != 0:
            temp_xy = self.rotate(temp_xy, (self.x, self.y), -self.angle)
        (x, y) = temp_xy
        flag1 = (self.x <= x <= self.x + self.horizontal and self.y - body_radius <= y <= self.y + self.vertical + body_radius)
        flag2 = (self.x - body_radius <= x <= self.x + self.horizontal + body_radius and self.y <= y <= self.y + self.vertical)
        flag3 = distance.euclidean(temp_xy, (self.x, self.y)) < body_radius
        flag4 = distance.euclidean(temp_xy, (self.x + self.horizontal, self.y)) < body_radius
        flag5 = distance.euclidean(temp_xy, (self.x + self.horizontal, self.y + self.vertical)) < body_radius
        flag6 = distance.euclidean(temp_xy, (self.x, self.y + self.vertical)) < body_radius
    ...
    def draw(self, ax):
        ...
        ax : matplotlib.pyplot.gca
            Current axes on this figure.
        """
        rectangle = patches.Rectangle(xy = (self.x, self.y), width = self.horizontal, height = self.vertical,
                                      angle = self.angle, fc = self.color, ec = self.color, alpha = 0.2)
    ...
    def save_text(self):
        ...
        ret : str
            Text of this sensor.
        """
        return "name = {}, index = {}, bottom left coordinates = ({}, {}), vertical = {}[cm], horizontal = {}[cm], angle = {}[deg]".format(self.name, self.index, self.x, self.y, self.vertical, self.horizontal, self.angle)
...
class CostSensor(Sensor):
    ...
    def save_text(self):
        ...
        ret : str
            Text of this sensor.
        """
        return "name = {}, index = {}, center coordinate = ({}, {})".format(self.name, self.index, self.x, self.y)
...
def check_indexes_of_sensors(sensors):
    ...
    for s in sensors:
        if s.index not in indexes:
            ...
            raise ValueError('Some indexes of sensors are same. Indexes of sensors must be different from each other.')
...
test_sensors = [
    ...
    CostSensor(FLOW_KITCHEN, 24, 600, -310, '#0000ff'),
    CostSensor(FLOW_BATHROOM, 25, 990, 60, '#0000ff'),
