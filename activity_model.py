...
class Activity:
    ...
    def sampling_duration_time(self, lower_limit=None, upper_limit=None, max_try=100):
        ...
        if upper_limit < lower_limit:
            raise ValueError(
                "The lower_limit must be lower than the upper_limit. If you don't specify each of them, lower_limit or upper_limit are defined as another way. See detail in the decostring of this functions."
            )
        ...
        while not is_valid:
            ...
            if try_count >= max_try:
                raise ValueError(
                    "Activity.sampling_duration_time() failed to sample duration time of {} {} times.".format(
                        self.__name, try_count
                    )
                )
...
class FundamentalActivity(Activity):
    ...
    def sampling_start_time(
        self, num_days, lower_limit=None, upper_limit=None, max_try=100
    ):
        """
        ...
        if (
            ...
            or (2 * 24 * 60 <= upper_limit)
        ):
            raise ValueError(
                "The lower_limit and upper_limit must satisfy the following conditions. lower_limit > 0, upper_limit < 2 * 24 * 60, and lower_limit <= upper_limit. If you don't specify each of them, lower_limit or upper_limit are defined as another way. See detail in the decostring of this functions."
            )
        ...
        while not is_valid:
            ...
            if try_count >= max_try:
                raise ValueError(
                    "FundamentalActivity.sampling_start_time() failed to sample start time of {} {} times.".format(
                        self.name, try_count
                    )
                )
...
class NecessaryActivity(Activity):
    ...
    def __init__(
        self, name, num_mean, duration_mean, duration_sd, place, color, home_equipment
    ):
        """
    ...
    def sampling_start_time(
        self, duration_time, remainder, rejection_range=None, max_try=100
    ):
        """
        ...
        while not is_valid:
            if try_count >= max_try:
                raise ValueError(
                    "NecessaryActivity.sampling_start_time() failed to sample start time of {} {} times.".format(
                        self.name, try_count
                    )
                )
            try_count += 1
            ...
            sub_start_seconds, sub_end_seconds = (
                sub_range[0].total_seconds(),
                sub_range[1].total_seconds(),
            )
            start_time_seconds = (
                rand() * (sub_end_seconds - sub_start_seconds) + sub_start_seconds
            )
            # prevent to generate a short time range
            if start_time_seconds - sub_start_seconds < rejection_seconds:
                continue
            if (
                start_time_seconds + duration_time.total_seconds() + rejection_seconds
                >= sub_end_seconds
            ):
                continue
            is_valid = True
    ...
    def sampling_start_time_with_constraints(
        self, duration_time, remainder, schedule, rejection_range=None, max_try=100
    ):
        """
        ...
        if self.exist_existence_constraints:
            sorted_schedule = sorted(schedule, key=lambda x: x[1])
            pre_time, post_time = (
                None,
                None,
            )  # end time of the pre activity, start time of the post activity
            ...
            for c in self.existence_constraints:
                ...
                for act in sorted_schedule:
                    if act[0].name == c[0].name:
                        pre_time = act[2]
                        is_pre = True
                    elif (act[0].name == c[1].name) and is_pre:
                        post_time = act[1]
                        break
                    elif (act[0].name == self.name) and is_pre:
                        satisfy_existence = True
                if (pre_time is None) or (post_time is None):
                    satisfy_existence = True
                if not satisfy_existence:
                    break
            if not satisfy_existence:
                for r in remainder:
        ...
        while not is_valid:
            if try_count >= max_try:
                raise ValueError(
                    "NecessaryActivity.sampling_start_time_with_constraints() failed to sample start time of {} {} times.".format(
                        self.name, try_count
                    )
                )
            try_count += 1
    ...
    def modified_new_activity(
        ...
        *,
        ...
        if home_equipment is None:
            home_equipment = self.home_equipment
        act = NecessaryActivity(
            name, num_mean, duration_mean, duration_sd, place, color, home_equipment
        )
...
class RandomActivity(Activity):
    def __init__(
        self, name, duration_mean, duration_sd, place, color, home_equipment, weight=1.0
    ):
        """
    ...
    def sampling_random_activity(activity_candidates):
        ...
        if activity_candidates == []:
            raise ValueError(
                "The activity_candidates must include at least one element."
            )
    ...
    def modified_new_activity(
        ...
        *,
        ...
        return RandomActivity(
            name, duration_mean, duration_sd, place, color, home_equipment, weight
        )
...
class MetaActivity:
    ...
    def sampling_sub_activities(
        self, start_time, lower_limit=None, upper_limit=None, max_try=100
    ):
        """
    ...
    def determine_place(self, existing_places):
        ...
        is_valid = True
        for sub_act in self.sub_activities:
            if sub_act.determine_place(existing_places) is None:
                is_valid = None
        return is_valid
...
class MetaFundamentalActivity(MetaActivity):
    ...
    def sampling_start_time(
        ...
        while not is_valid:
            ...
            if try_count >= max_try:
                raise ValueError(
                    "MetaFundamentalActivity.sampling_start_time() failed to sample start time of {} {} times.".format(
                        self.__name, try_count
                    )
                )
...
class MetaNecessaryActivity(MetaActivity):
    ...
    def sampling_start_time(
        ...
        while not is_valid:
            if try_count >= max_try:
                raise ValueError(
                    "MetaNecessaryActivity.sampling_start_time() failed to sample start time of {} {} times.".format(
                        self.name, try_count
                    )
                )
            try_count += 1
...
def determine_activity_set(path, original_act_model):
    ...
    new_act_model["fundamental_activities"] = [
        a
        for a in new_act_model["fundamental_activities"]
        if a.determine_place(existing_places) is not None
    ]
    new_act_model["necessary_activities"] = [
        a
        for a in new_act_model["necessary_activities"]
        if a.determine_place(existing_places) is not None
    ]
    new_act_model["random_activities"] = [
        a
        for a in new_act_model["random_activities"]
        if a.determine_place(existing_places) is not None
    ]
...
have_breakfast = FundamentalActivity(
    "Have breakfast",
    timedelta(hours=7, minutes=17) / timedelta(minutes=1),
    30,
    34,
    10,
...
have_lunch = FundamentalActivity(
    "Have lunch",
...
have_dinner = FundamentalActivity(
    "Have dinner",
    timedelta(hours=18, minutes=18) / timedelta(minutes=1),
    30,
    45,
    10,
    ["Dinner_Table_Chair", "Entrance"],
...
sleep = FundamentalActivity(
    "Sleep",
    timedelta(hours=21, minutes=29) / timedelta(minutes=1),
    40,
    482,
    30,
    ["Bed"],
...
take_a_bath = NecessaryActivity(
    ...
    ["Bathroom_Door"],
    "#00ffff",
    {constants.FLOW_BATHROOM: 1},
)
change_clothes = NecessaryActivity(
    "Change clothes", 2, 5, 1, ["Wardrobe", "Bed"], "#008000", {}
)
brush_teeth = NecessaryActivity(
    "Brush teeth",
    2,
    1.5 + 0.5,
    0.5,
...
go_out = RandomActivity(constants.GO_OUT_NAME, 40, 20, ["Entrance"], "#ffe4c4", {})
cooking = RandomActivity(
    ...
    ["Kitchen_Stove"],
    "#ff0000",
    {constants.POWER_KITCHEN: 1, constants.FLOW_KITCHEN: 1},
)
clean = RandomActivity("Clean", 30, 10, ["Trash_Bin"], "#bc8f8f", {})
do_laundry = RandomActivity("Do laundry", 5, 1, ["Wash_Machine"], "#008080", {})
watch_TV = RandomActivity(
    "Watch TV", 40, 20, ["Sofa"], "#7cfc00", {constants.POWER_TV: 1}
)
read = RandomActivity("Read", 40, 20, ["Desk_Chair", "Desk"], "#808000", {})
take_a_snack = RandomActivity(
    "Take a snack", 10, 3, ["Dinner_Table_Chair", "Sofa"], "#ff69b4", {}
)
use_the_phone = RandomActivity(
    "Use the phone", 10, 3, ["Desk_Chair", "Desk", "Bed"], "#800000", {}
)

# extra activities
wandering = NecessaryActivity(constants.WANDERING_NAME, 0, 5, 1, [], "#ffff00", {})
...
go_out_necessary = NecessaryActivity(
    go_out.name,
    4,
    go_out.duration_mean,
    go_out.duration_sd,
    go_out.place,
    go_out.color,
    go_out.home_equipment,
)
use_the_phone_necessary = NecessaryActivity(
    use_the_phone.name,
    1,
    use_the_phone.duration_mean,
    use_the_phone.duration_sd,
    use_the_phone.place,
    use_the_phone.color,
    use_the_phone.home_equipment,
)
...
applied_have_lunch_0 = Activity(
    "(sub 1) Take foods", 0.5, 0.001, ["Refrigerator", "Kitchen_Stove"], "#ffdacc", {}
)
applied_have_lunch_1 = Activity("(sub 2) Cook", 15, 3, ["Kitchen_Stove"], "#ffb499", {})
applied_have_lunch_2 = Activity(
    "(sub 3) Take tableware", 0.4, 0.001, ["Cupboard", "Kitchen_Stove"], "#ff8f66", {}
)
applied_have_lunch_3 = Activity("(sub 4) Eat", 30, 5, ["Dinner_Table"], "#ff6933", {})
applied_have_lunch_4 = Activity(
    "(sub 5) Wash tableware", 5, 0.1, ["Kitchen_Stove"], "#ff4500", {}
)
applied_have_lunch_list = [
    applied_have_lunch_0,
    applied_have_lunch_1,
    applied_have_lunch_2,
    applied_have_lunch_3,
    applied_have_lunch_4,
]
applied_have_breakfast = MetaFundamentalActivity(
    "Applied Have breakfast",
    applied_have_lunch_list,
    timedelta(hours=7, minutes=17) / timedelta(minutes=1),
    30,
)
applied_have_lunch = MetaFundamentalActivity(
    "Applied Have lunch",
    applied_have_lunch_list,
    timedelta(hours=12) / timedelta(minutes=1),
    30,
)
applied_have_dinner = MetaFundamentalActivity(
    "Applied Have dinner",
    applied_have_lunch_list,
    timedelta(hours=18, minutes=18) / timedelta(minutes=1),
    30,
...
applied_take_a_bath_1 = Activity(
    "(sub 1) Take a bath",
    30,
    10,
    ["Bathroom_Door"],
...
applied_take_a_bath_2 = Activity(
    "(sub 2) Wash clothes", 1, 0.1, ["Wash_Machine"], "#008080", {}
)
applied_take_a_bath_list = [applied_take_a_bath_1, applied_take_a_bath_2]
applied_take_a_bath = MetaNecessaryActivity(
    "Applied Take a bath", applied_take_a_bath_list, 1
)
...
applied_go_out_2 = Activity(
    f"(sub 2) {constants.GO_OUT_NAME}", 40, 20, ["Entrance"], "#ffe4c4", {}
)
...
applied_urination = NecessaryActivity(
    "Urination", 5, 2, 0.5, ["Toilet_Door"], "#6a5acd", {}
)
applied_urination.set_repulsive_constraints([("Urination", timedelta(hours=1))])

applied_brush_teeth = NecessaryActivity(
    "Brush teeth",
    2,
    1.5,
    0.5,
...
applied_activity_model = {
    "fundamental_activities": [
        applied_have_breakfast,
        applied_have_lunch,
        applied_have_dinner,
        sleep,
    ],
...
applied_urination.set_repulsive_constraints([("Urination", timedelta(hours=1))])
applied_defecation = NecessaryActivity(
    "Defecation", 1, 10, 3, ["Toilet_Door"], "#4b0082", {}
)
...
applied_brush_teeth.set_existence_constraints(
    [(have_dinner, sleep), (sleep, have_breakfast)]
)

applied_change_clothes = NecessaryActivity(
    "Change clothes", 2, 2, 1, ["Wardrobe"], "#008000", {}
)
applied_change_clothes.set_existence_constraints(
    [(have_dinner, sleep), (sleep, have_breakfast)]
)
